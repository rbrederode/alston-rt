...
from models.obs import ObsState
...
class Observation:
    ...
    def __init__(self,
        ...
        with self._rlock:
            ...
            self.id = obs_id or generate_obs_id()
            self.status = ObsState.EMPTY  # Initial status of the observation

            self.short_desc = desc     # Short description of the observation
            self.long_desc = desc      # Long description of the observation
            ...
            self.duration = duration        # Total time on target (seconds)

            self.sample_count = self.sample_rate * self.duration  # Total number of samples to be collected

            self._freq_min = None           # Start of frequency scanning (Hz)
            self._freq_max = None           # End of frequency scanning (Hz)
            self._freq_scans = None         # Number of frequency scans
            self._freq_overlap = None       # Overlap between frequency scans (Hz)
            self._freq_duration = None      # Duration of each frequency scan (seconds)
            self._scan_iterations = None    # Number of scan iterations (within a frequency scan)
            self._scan_duration = None      # Duration of each scan (seconds)

            # Calculate and populate above scan parameters
            self.determine_scans()

            self.start = None       # Scheduled start date time of observation
            self.end = None         # Scheduled end date time of observation
            ...
            self._init_data_arrays(freq_scans=self._freq_scans, scan_iters=self._scan_iterations, scan_duration=self._scan_duration, channels=self.channels)
    ...
    def _init_data_arrays(self, freq_scans:int, scan_iters:int, scan_duration:int, channels:int):
        ...
        with self._rlock:
            ...
            self.itpw = np.zeros((freq_scans, channels), dtype=np.float64)  # Initialise (scan X fft_size) array for total power data
    ...
    def determine_scans(self):
        ...
        """
        if self.freq_ctr is None or self.bandwidth is None or self.sample_rate is None or self.duration is None:
            raise XSoftwareFailure(f"Observation {self.id} - Center frequency {self.freq_ctr}, bandwidth {self.bandwidth}, "
                                   f"sample rate {self.sample_rate}, and duration {self.duration} must be set to determine scans.")

        with self._rlock:
            self._freq_min = self.freq_ctr - self.bandwidth / 2 - self.sample_rate * (1-USABLE_BANDWIDTH)/2  # Start of frequency scanning
            self._freq_max = self.freq_ctr + self.bandwidth / 2 + self.sample_rate * (1-USABLE_BANDWIDTH)/2  # End of frequency scanning

            self._freq_scans = int(-((self._freq_max-self._freq_min)) // -(self.sample_rate * USABLE_BANDWIDTH))  # Ceiling division
            # Overlap in the frequency domain (Hz) rounded to 4 decimals
            self._freq_overlap = round(((self.sample_rate * self._freq_scans - (self._freq_max-self._freq_min))/(self._freq_scans-1) if self._freq_scans > 1 else 0),4)
            self._freq_duration = round(self.duration / self._freq_scans,3) if self._freq_scans > 0 else self.duration
            self._scan_iterations = int(np.ceil(self._freq_duration / MAX_SCAN_DURATION))  # Number of iterations of a frequency scan, # e.g. 5*60 seconds in a freq scan will be 5 scans of 1 minute each
            self._scan_duration = self._freq_duration // self._scan_iterations if self._scan_iterations > 1 else self._freq_duration  # Duration of each scan in seconds

        return self._freq_scans, self._freq_overlap, self._scan_iterations, self._scan_duration
    ...
    def schedule(self, start: datetime, end: datetime):
        ...
        if duration <= self.duration * SCAN_OVERHEAD_FACTOR:
    ...
    def get_metadata(self) -> dict:
        ...
        with self._rlock:
            ...
            for i in range(self._freq_scans):

                scan_freq_meta = {
                    "scan_freq_num": i,
                    "scan_freq_min": self._freq_min + (i * (self.sample_rate - self._freq_overlap)),
                    "scan_freq_ctr": self._freq_min + (i * (self.sample_rate - self._freq_overlap)) + self.sample_rate / 2,
                    "scan_freq_max": self._freq_min + (i * (self.sample_rate - self._freq_overlap)) + self.sample_rate,
                    "scan_freq_duration": self._scan_duration * self._scan_iterations,
                    "scan_iters": []
                }

                for j in range(self._scan_iterations):
                    scan_iter_meta = {
                        "scan_iter_num": j,
                        "scan_iter_duration": self._scan_duration
                    }
            ...
            obs_meta = {
                ...
                "target": {
                    "name": self.target.name if self.target else None,
                ...
                "freq_scan_duration": self._scan_duration * self._scan_iterations,
                "scan_iterations": self._scan_iterations,
                "scan_iter_duration": self._scan_duration,
                "status": self.status,
                "start": self.start.isoformat() if self.start else None,
                "end": self.end.isoformat() if self.end else None,
                "scans": scans_meta["scan_freqs"]
            }
    ...
    def load_baselines(self, input_dir: str) -> bool:
        ...
        for idx, scan_freq in enumerate(metadata.get("scans", [])):
            ...
            if load_files:
                ...
                logger.info(
                    f"Observation {self.id} - Loading baseline power spectrum {load_file} with center freq {scan_freq_cf}, "
                ...
                self.ibsl[idx, :] = np.ones((self.channels,))
                logger.warning(
                    f"Observation {self.id} - No load file for frequency scan {idx} in {input_dir}. "
